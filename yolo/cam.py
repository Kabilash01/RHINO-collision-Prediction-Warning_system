...
if camera_index == -1:
    print("[FATAL] No available webcam found.")
    sys.exit(1)

cap = cv2.VideoCapture(camera_index)
print(f"[INFO] Using webcam at index {camera_index}")
# Optional: Set frame size if supported
# cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
# cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)

if not cap.isOpened():
    print("[ERROR] Failed to open detected webcam.")
    exit()
