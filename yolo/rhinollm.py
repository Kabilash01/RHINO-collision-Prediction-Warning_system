...
from llm_handler import generate_alert_message, generate_collision_explanation, get_voice_command, speak_text, get_route
...
while cap.isOpened():
    ...
    if (forecast[0] > 0.3 and headway < 20.0 and ttc < prt + 1) or crash_flag:
        ...
        if "route" in driver_command or "direction" in driver_command:
            ...
            summary = get_route(destination)
            speak_text(summary)
    else:
        cv2.putText(frame, "✓ SAFE", (30, 160), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

    cv2.putText(frame, f"VSV: {vsv:.1f} VLV: {vlv:.1f} Headway: {headway:.1f}", (30, 105), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
    cv2.putText(frame, f"Weather: {visibility}", (30, 135), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 255, 255), 1)
    cv2.putText(frame, f"Temp: {temperature:.1f}C  Humidity: {humidity:.1f}%", (30, 190), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 1)
    cv2.putText(frame, f"Distance: {sensor_value:.1f} mm", (30, 220), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
